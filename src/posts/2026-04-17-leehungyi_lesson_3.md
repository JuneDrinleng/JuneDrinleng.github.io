...
![image-20260417172236159](./assets/2026-04-17-leehungyi_lesson_3.assets/image-20260417172236159.png)
...
![image-20260417172444561](./assets/2026-04-17-leehungyi_lesson_3.assets/image-20260417172444561.png)

但这里算到的内容是$a_i$，他是要计算出一个0，1区间的权重，理论上是用上面的式子，但实际上还需要减去$A_{max}$

![image-20260417172514371](./assets/2026-04-17-leehungyi_lesson_3.assets/image-20260417172514371.png)

需要注意的是，不能直接把$a_i$放在工作台上，这是因为L有可能很大很大几百万这样，所以工作台不能放L相关的内容，这就使得找$a_{max}$变得很繁琐，需要每个chunk里寻找然后逐步更新

![image-20260417172841682](./assets/2026-04-17-leehungyi_lesson_3.assets/image-20260417172841682.png)

每个chunk进行一个比较d和chunk内元素大小的情况

所以理论上这个比较大小的运算要进行$B=\frac{L}{N}$次才能找到$a_{max}$，找到后就可以每个元素逐步计算出每一项分子：

![image-20260417173046477](./assets/2026-04-17-leehungyi_lesson_3.assets/image-20260417173046477.png)

而分母需要计算出总和，也是每个chunk逐步计算：

![image-20260417173119958](./assets/2026-04-17-leehungyi_lesson_3.assets/image-20260417173119958.png)

求出坟墓之后才能算出$\hat{a_i}$:

![image-20260417173231655](./assets/2026-04-17-leehungyi_lesson_3.assets/image-20260417173231655.png)

这一段操作才算计算完，这中间搬了很多次from HBM，但这还没完，还需要搬运v来计算o的局部：

![image-20260417173412148](./assets/2026-04-17-leehungyi_lesson_3.assets/image-20260417173412148.png)

那么问题来了，flash attention想解决的就是怎么简化这些反复的读写？

### 1.3 简化版本的flash attention

![image-20260417173717038](./assets/2026-04-17-leehungyi_lesson_3.assets/image-20260417173717038.png)

假设第一个chunk的话，$d_1$就是$a_{max}$先计算一通

那么到了第二个chunk，假设遇到了更大的$d_2$，那么基于$d_2$作为$a_{max}$来计算：

![image-20260417173857141](./assets/2026-04-17-leehungyi_lesson_3.assets/image-20260417173857141.png)

这里怎么直接根据$d_1$ and $d_2$来修正$s_1$呢？

![image-20260417173954084](./assets/2026-04-17-leehungyi_lesson_3.assets/image-20260417173954084.png)

这样就可以实现消除$d_1$的痕迹，这1就是flash attention的核心技巧。这样所有处理后，$a_{max}$和分母s都有，这样就很好计算了：

![image-20260417174443053](./assets/2026-04-17-leehungyi_lesson_3.assets/image-20260417174443053.png)
...
![image-20260417174818200](./assets/2026-04-17-leehungyi_lesson_3.assets/image-20260417174818200.png)

即便这里的$s_1\neq s_{real}$,  $d_1 \neq a_{max}$

那么我们看第二关chunk，之前已经说到了更新s的方案，但对于flash attention来说，更重要的是怎么直接算对o：

![image-20260417175125358](./assets/2026-04-17-leehungyi_lesson_3.assets/image-20260417175125358.png)

这里新的attention weight 的$o_2$需要怎么抹掉$o_1$里$s_1$ and $d_1$存在的痕迹：

![image-20260417175306694](./assets/2026-04-17-leehungyi_lesson_3.assets/image-20260417175306694.png)

先通过乘以$\frac{s_1}{s_2}$ 来更新分母，再通过乘以$(e^{d_1-d_2})$来更新分子，通过这种迭代的方式来不断修正前面的错误：

![image-20260417175507509](./assets/2026-04-17-leehungyi_lesson_3.assets/image-20260417175507509.png)
...
![image-20260417181443992](./assets/2026-04-17-leehungyi_lesson_3.assets/image-20260417181443992.png)

对于KV Cache，在计算attention的时候，k和v是被存下来而非丢掉。这是因为模型在生成的时候，新的输入是接前面的prompt的，这样就可以直接利用前面的attention就行了：

前面已经算了attention：

![image-20260417181718843](./assets/2026-04-17-leehungyi_lesson_3.assets/image-20260417181718843.png)

那么对于第四个token，就可以直接使用前面的kv了：

![image-20260417181736547](./assets/2026-04-17-leehungyi_lesson_3.assets/image-20260417181736547.png)
...
![image-20260417181944647](./assets/2026-04-17-leehungyi_lesson_3.assets/image-20260417181944647.png)
...
![image-20260417182227054](./assets/2026-04-17-leehungyi_lesson_3.assets/image-20260417182227054.png)

![image-20260417182234451](./assets/2026-04-17-leehungyi_lesson_3.assets/image-20260417182234451.png)

Group-query Attention是被广泛使用的，llama,Gemma都是通过这个实现的

### 2.2 Multi-head Latent Attention

![image-20260417182413918](./assets/2026-04-17-leehungyi_lesson_3.assets/image-20260417182413918.png)

另一个方法就是先把x压成一个压缩向量，然后后续再乘回kq，这些都是需要训练的，需要注意的是这个方法是无需解压缩的

![image-20260417182708916](./assets/2026-04-17-leehungyi_lesson_3.assets/image-20260417182708916.png)
...
![image-20260417183144328](./assets/2026-04-17-leehungyi_lesson_3.assets/image-20260417183144328.png)
...
![image-20260417183624485](./assets/2026-04-17-leehungyi_lesson_3.assets/image-20260417183624485.png)

当然另一个方法就是有些layer是完整的attention，有的layer则sliding：

![image-20260417183705518](./assets/2026-04-17-leehungyi_lesson_3.assets/image-20260417183705518.png)

虽然sliding会使得模型变差，但只要在inference的时候额外的加上最前面的几个token就可以使得模型的表现大增：

![image-20260417183920032](./assets/2026-04-17-leehungyi_lesson_3.assets/image-20260417183920032.png)
...
![image-20260417184306121](./assets/2026-04-17-leehungyi_lesson_3.assets/image-20260417184306121.png)
...
![image-20260417184629379](./assets/2026-04-17-leehungyi_lesson_3.assets/image-20260417184629379.png)
...
![image-20260417185049472](./assets/2026-04-17-leehungyi_lesson_3.assets/image-20260417185049472.png)

## 3 Conclusion

![image-20260417185321457](./assets/2026-04-17-leehungyi_lesson_3.assets/image-20260417185321457.png)
