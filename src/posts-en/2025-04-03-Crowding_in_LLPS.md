...
Judgment of crowding in two phases of phase separation often relates to R and the radius of gyration Rg:
...
1. **Top-left: Colloid Limit (Rg < R)**
   - **Large black sphere is a colloid particle**, small curved line is a polymer chain.
   - Because **Rg < R**, i.e., the size of the polymer chain is smaller than the colloid, **the colloid becomes the object of crowding**, while the polymer is the **crowder (crowding agent)**.
   - This is what you mentioned: **macromolecules as crowders**.
2. **Middle-top: Rg ∼ R**
   - Polymer chains and colloids are of similar size, competition for space between them becomes significant.
   - At this scale, crowding is more complex and cannot be simply distinguished who is the crowder.
3. **Top-right: Protein Limit (Rg > R)**
   - Polymer chains are large, colloids become small points (similar to protein particles).
   - Colloids (e.g., proteins) become the **crowders**, while the polymer chain is the restricted object.

The correlation length ξ describes the microscopic scale characteristics of crowding

Wang et al. [(61)](javascript:void(0);) stated that crowding can affect protein stability in two ways: by hard-core repulsions or by soft chemical interactions. ——crowding can affect protein stability through two ways: hard-core repulsions or soft chemical interactions
...
The explicit contraposition of entropy and enthalpy in the effect of crowders on protein stability was also accepted by other authors. For example, Senske et al., [(63)](javascript:void(0);) when studying the thermal unfolding of ubiquitin, observed that addition of other solutes (glucose, dextran, PEG, guanidinium, and urea) led to both enthalpic and entropic destabilization. The authors argued that the classification of cosolute effects based on their excess enthalpic contributions results in a comprehensive thermodynamic model. Other authors also accepted the effect of entropy and enthalpy on protein stability. For example, Senske et al. [(63)](javascript:void(0);) when studying the thermal unfolding of ubiquitin, observed that addition of other solutes (glucose, dextran, PEG, guanidinium, and urea) led to both enthalpic and entropic destabilization. The authors argued that the classification of cosolute effects based on their excess enthalpic contributions results in a comprehensive thermodynamic model.

> Why is it difficult to form LLPS after adding urea to BSA LLPS?

Regarding the view of crowding, some believe it should be inert, not interacting with the target molecule, so PEG, DEXTRAN, FICOLL, and POLY(4-benzoate) (PSS) are usually selected. These polymers are typically provided as polydisperse species, thus having a molecular mass distribution—entropy-enthalpy separation, mainly entropy-dominated?

Another view is that crowding actually has weak interactions with the target molecule, such as Lee et al. studied the interaction between PEG and specific bases of proteins—still having enthalpic effects
...
Some researchers selected classic LLPS protein systems: Examples of these proteins are the bovine pancreatic trypsin inhibitor (BPTI), ribonuclease A, lysozyme, β-lactoglobulin, hemoglobin, and bovine serum albumin (BSA)

![img](https://raw.githubusercontent.com/JuneDrinleng/JuneDrinleng.github.io/main/assets/img/cr3c00615_0003.jpeg)

Tight folding of proteins reduces their spatial repulsion with surrounding crowders, thereby lowering the free energy

![Figure 1](https://raw.githubusercontent.com/JuneDrinleng/JuneDrinleng.github.io/main/assets/img/cr3c00615_0005.gif)

Hydrogen bond structures also affect thermodynamic stability—amyloid proteins
...
You can refer to this image for drawing
...
The A1 protein consists of `evolutionarily conserved domains` and `linker domains between them`. Linkers primarily respond to pH and isoelectric point changes, while conserved regions may exhibit non-specific hydrophobic interactions.
...
For the effects of pH and concentration on phase separation, many proteins can generate phase diagrams:
