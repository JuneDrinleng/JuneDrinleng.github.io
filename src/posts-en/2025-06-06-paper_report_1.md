---
layout: post
title: "20250606 Literature Reading Notes"
date: 2025-06-06
tags: [paper]
comments: true
author: junedrinleng
toc: true
post_id: 2025-06-06-paper_report_1
source_lang: zh
source_title: "20250606文献日读笔记"
title_key: literature_note
generated_by: scripts/i18n-sync.mjs
generated_at: "2026-03-09T04:22:43.290Z"
source_hash: 42a06a0f6fa4848820721f8efa15b0bf1b5d041181b62c1a4a61535f359cdb29
---

This note mainly summarizes part of the literature read on June 6, 2025, including a review on sodium-sulfur batteries and their mechanisms, a study on spectral response of velocity fields using coordination complexes + cross-linked networks, and cellulose scaffold-based hydrogels

<!-- more -->

## 1 Utilizing Cellulose Scaffold to Achieve a Robust and Controllable Soft Actuator with a Heterogeneous Interface Interlocked Hydrogel

> reference: [Y. Song *et al.*, *Advanced Functional Materials*. **n/a**, 2509712 (2025).](https://advanced.onlinelibrary.wiley.com/doi/10.1002/adfm.202509712)

![65fda325e47fe2bcd9ad57e4cb31a22](https://raw.githubusercontent.com/JuneDrinleng/JuneDrinleng.github.io/main/assets/img/65fda325e47fe2bcd9ad57e4cb31a22.png)

The material is inspired by leaf structures, with the entire hydrogel undergoing in-situ polymerization on the cellulose scaffold, exhibiting conductivity, stress-strain curves, damping effects, and other mechanical properties

![de58c3715bbada03ca9617e81544ed9](https://raw.githubusercontent.com/JuneDrinleng/JuneDrinleng.github.io/main/assets/img/de58c3715bbada03ca9617e81544ed9.png)

The material can transition from a relatively soft state to hardening, and its tensile strain performance was tested

![93d435a544432b24ffad378121f32eb](https://raw.githubusercontent.com/JuneDrinleng/JuneDrinleng.github.io/main/assets/img/93d435a544432b24ffad378121f32eb.png)

Immersion in different concentrations of trivalent iron ions leads to gradual bending (related to salting-out effects, with different $Fe^{3+}$ environments causing varying water content in the hydrogel), while in hydrochloric acid solutions, it exhibits dynamic dissociation

![949ff607dfada406fc2c293afe78dd1](https://raw.githubusercontent.com/JuneDrinleng/JuneDrinleng.github.io/main/assets/img/949ff607dfada406fc2c293afe78dd1.png)

Using different particles also affects the bending angle of the hydrogel material. Compared to the control group, PBCI material maintains better bending deformation after de-crosslinking and re-crosslinking, showing good stability against pressure and cycling (Figures g,h)

## 2 Stress Fluorescence Visualization - Using Covalent Organic Frameworks

> reference: Y. Ren *et al.*, *J. Am. Chem. Soc.* (2025), doi:[10.1021/jacs.5c05354](https://doi.org/10.1021/jacs.5c05354)

![b3f3efb618ac99f70ea20d57fe5da7b](https://raw.githubusercontent.com/JuneDrinleng/JuneDrinleng.github.io/main/assets/img/b3f3efb618ac99f70ea20d57fe5da7b.jpg)

Ruthenium complexes' amino groups react with TPE-4CHO's aldehyde groups to form imine-linked cross-linked networks

b) Calculated surface charge distribution (DFT), then measured fluorescence spectra at different concentrations

![img](https://raw.githubusercontent.com/JuneDrinleng/JuneDrinleng.github.io/main/assets/img/ja5c05354_0002.jpeg)

Adding end-capped PEG to achieve PEGylation, leading to secondary self-assembly of supramolecular structures, with PEG's reaction and spectral characteristics varying accordingly

![img](https://raw.githubusercontent.com/JuneDrinleng/JuneDrinleng.github.io/main/assets/img/ja5c05354_0003.jpeg)

The material also responds to strain and friction, with different flow velocities in pipes showing distinct responses:

![img](https://raw.githubusercontent.com/JuneDrinleng/JuneDrinleng.github.io/main/assets/img/ja5c05354_0004.jpeg)

Thus, it can be used for in vivo brain blood flow velocity imaging

(from Yang Zhenzhong's group **can have a try?** Check his subsequent citations first)

## 3 Manipulating Sulfur Redox Kinetics in Rechargeable Metal-Sulfur Batteries: Fundamental Principles and General Methods /think
A Review on the Redox Kinetics within Sulfur-Based Batteries

> reference: X.-L. Huang *et al.*, , doi:[10.1002/adma.202419089](https://doi.org/10.1002/adma.202419089).

![Details are in the caption following the image](https://raw.githubusercontent.com/JuneDrinleng/JuneDrinleng.github.io/main/assets/img/adma202419089-fig-0003-m.jpg)

Sulfur electrodes undergo multiple levels of redox processes, from $S_8$ elemental form to various chain-like polysulfide ions $S_x^{y-}$

![Details are in the caption following the image](https://raw.githubusercontent.com/JuneDrinleng/JuneDrinleng.github.io/main/assets/img/adma202419089-fig-0004-m.jpg)

![Details are in the caption following the image](https://raw.githubusercontent.com/JuneDrinleng/JuneDrinleng.github.io/main/assets/img/adma202419089-fig-0006-m.jpg)

Illustration of the electron transfer mechanism for polysulfide ions

![Details are in the caption following the image](https://raw.githubusercontent.com/JuneDrinleng/JuneDrinleng.github.io/main/assets/img/adma202419089-fig-0009-m.jpg)

For polymer electrolytes, cations exhibit bridging behavior and form salt effects, leading to dissociation of tightly bound ion pairs. High salt concentration results in more localized ion pairs, reducing conductivity

![Details are in the caption following the image](https://raw.githubusercontent.com/JuneDrinleng/JuneDrinleng.github.io/main/assets/img/adma202419089-fig-0010-m.jpg)
