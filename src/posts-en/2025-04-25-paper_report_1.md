...
A reversible hydrogel network with a catalyst promoting the transition of the hydrogel network
...
A water-based phase separation induced by dextran and PNIPAM, with viscoelasticity changing with temperature:
...
Reconstruction using dynamic time series information and original images, implemented with tensorflow

Hybrid SDE implementation, hidden state numbers need to be pre-set
...
Time series (x_i) from different regions/characteristics, constructing D(x) matrix to achieve model-free inference of interactions between nodes
...
Confocal capturing disordered polymorphic motion, calculating autocorrelation (zero-mode waveguides (ZMWs) assistance), corresponding conformational change interpretation (denaturation)
